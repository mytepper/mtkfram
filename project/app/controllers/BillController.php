...
class BillController extends BaseController{
    ...
    Public function addlist(){
      //Session::push('bill', Input::all());
      //Session::put('bill', array_add($bill = Session::get('bill'), Input::all()));
      //return json_encode(Session::get('bill'));
      //Session::flush();
    ...
    public function add(){
                  ...
                  if($db){
                        ...
                        return Redirect::to('store/bill/add')->with('success', '
    ...
    public function edit(){
                    ...
                    if(count(Input::get('PID'))>0){
                      BillList::where('bill_id',$id)->delete(); //ลบข้อมูลเก่า
                      $PID = Input::get('PID');
                      $dataSet = [];
                      foreach ($PID as $key => $value) {
                          $dataSet[] = [
                   ...
                   if($db){
                            return Redirect::to('store/bill/edit/'.$id)->with('success', '
                    <div class="alert alert-success alert-dismissable">
                      <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                      <i class="fa fa-warning pr10"></i>
                      บันทึกข้อมูลแล้ว
    ...
    public function pdf($id){
      $rs = Bill::one($id);
      $html = View::make('store/bill/pdf')->with('rs',$rs)->render();
