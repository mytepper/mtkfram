...
class ReceiveController extends BaseController{
    ...
    Public function addlist(){
      //Session::push('receive', Input::all());
      //Session::put('receive', array_add($receive = Session::get('receive'), Input::all()));
      //return json_encode(Session::get('receive'));
      //Session::flush();
    ...
    public function add(){
                  ...
                  if($db){
                        ...
                        return Redirect::to('store/receive/add')->with('success', '
    ...
    public function edit(){
                     $id = Input::get('id');
                     $db = Receive::where('id', $id)
                    ->update(array(
                        'CUID' => Input::get('CUID'),
                    ));
                    
                    if(count(Input::get('PID'))>0){
                      ReceiveList::where('bill_id',$id)->delete(); //ลบข้อมูลเก่า
                      $PID = Input::get('PID');
                      $dataSet = [];
                      foreach ($PID as $key => $value) {
                          $dataSet[] = [
                   ...
                   if($db){
                            return Redirect::to('store/receive/edit/'.$id)->with('success', '
                    <div class="alert alert-success alert-dismissable">
                      <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                      <i class="fa fa-warning pr10"></i>
                      บันทึกข้อมูลแล้ว
    ...
    public function pdf($id){
      $rs = Receive::one($id);
      $html = View::make('store/receive/pdf')->with('rs',$rs)->render();
